...
class DecodeDetections(Layer):
    ...
    def call(self, y_pred, mask=None):
        ...
        # Convert anchor box offsets to image offsets. I think the variance is an important hyper-parameters. It should be chosen carefully.
        ...
        def filter_predictions(batch_item):
            ...
            def filter_single_class(index):
                ...
                # a tensor of shape (n_boxes, 1 class id + 1 class conf + 4 coordinates) that contains the
                # confidnece values for just one class, determined by `index`.
                confidences = tf.expand_dims(batch_item[..., index], axis=-1) # a tensor of shape (n_boxes, 1 class)
                class_id = tf.fill(dims=tf.shape(confidences), value=tf.to_float(index))
                box_coordinates = batch_item[...,-4:] ## a tensor of shape (n_boxes, 4 coordinates)

                single_class = tf.concat([class_id, confidences, box_coordinates], axis=-1) # a tensor of shape (n_boxes, 1 class id + 1 class conf + 4 coordinates)
